...
namespace EV3.Dev.Csharp
{
	public static class Inputs
	{
		public const string INPUT_1 = "ev3-ports:in1"; //!< Sensor port 1
		public const string INPUT_2 = "ev3-ports:in2"; //!< Sensor port 2
		public const string INPUT_3 = "ev3-ports:in3"; //!< Sensor port 3
		public const string INPUT_4 = "ev3-ports:in4"; //!< Sensor port 4
	}

	public static class Outputs
	{
		public const string OUTPUT_A = "ev3-ports:outA"; //!< Motor port A
		public const string OUTPUT_B = "ev3-ports:outB"; //!< Motor port B
		public const string OUTPUT_C = "ev3-ports:outC"; //!< Motor port C
		public const string OUTPUT_D = "ev3-ports:outD"; //!< Motor port D
	}
	...
	public class Device
	{
			   ...
			   IDictionary<string, string[]> match)
		{
			...
			foreach (var currentFullDirPath in dirs)
			{
				...
				if (dirName.StartsWith(pattern))
				{
					...
					Console.WriteLine($"\t- Found '{_path}' device");
					bool bMatch = true;
		...
		{
			return new StreamReader(new FileStream(Path.Combine(_path, name), FileMode.Open, FileAccess.Read, FileShare.Read));
		}

		private StreamWriter OpenStreamWriter(string name)
		{
			return new StreamWriter(new FileStream(Path.Combine(_path, name), FileMode.Create, FileAccess.Write, FileShare.Write));
		}
